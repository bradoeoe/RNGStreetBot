...
class GunGame(commands.Cog):
    ...
    async def knife(self, ctx: commands.Context, discord_name: str, proof: discord.Attachment):
        ...
        cursor.execute("SELECT opponent_id, level FROM gun_game WHERE discord_id = ?", (user_id,))
        result = cursor.fetchone()
        opponent_id, current_level = result
        if not result:
        ...
        if opponent_id is None:
            await ctx.reply(f"<@{user_id}> currently has no opponent so they cannot be knifed. Try again tomorrow :)")
            conn.close()
            return
