...
class GunGame(commands.Cog):
    def __init__(self, bot):
        ...
        for guild in self.bot.guilds:
            ...
            cursor.execute(
                """CREATE TABLE IF NOT EXISTS gun_game (
            ...
            cursor.execute(
                """CREATE TABLE IF NOT EXISTS gun_game_bosses (
                                id INTEGER PRIMARY KEY,
                                name TEXT
                            )"""
            )
            ...
            cursor.execute(
                """INSERT OR IGNORE INTO game_state (id, is_live) VALUES (1, 0)"""
            )
            conn.commit()
            conn.close()

    @commands.hybrid_command(
        name="start_game", description="Start the game and set it as live."
    )
    async def start_game(self, ctx: commands.Context, codeword: str):
        ...
        for i in range(0, len(participants), 2):
            if i + 1 < len(participants):
                ...
                cursor.execute(
                    "UPDATE gun_game SET level = 1, opponent_id = ? WHERE discord_id = ?",
                    (p2, p1),
                )
                cursor.execute(
                    "UPDATE gun_game SET level = 1, opponent_id = ? WHERE discord_id = ?",
    ...
    @commands.hybrid_command(
        name="kill_confirmed", description="Confirm a kill in the gun game."
    )
    ...
    async def kill_confirmed(self, ctx: commands.Context, proof: discord.Attachment):
        ...
        cursor.execute(
            "SELECT opponent_id, level FROM gun_game WHERE discord_id = ?",
            (ctx.author.id,),
        )
        result = cursor.fetchone()
        if not result:
        ...
        cursor.execute(
            'UPDATE gun_game SET bracket = "loser", opponent_id = NULL WHERE discord_id = ?',
            (opponent_id,),
        )
        ...
        if waiting_loser:
            ...
            cursor.execute(
                "UPDATE gun_game SET opponent_id = ? WHERE discord_id = ?",
                (waiting_loser_id, opponent_id),
            )
            cursor.execute(
                "UPDATE gun_game SET opponent_id = ? WHERE discord_id = ?",
                (opponent_id, waiting_loser_id),
            )
        ...
        if new_opponent:
            ...
            cursor.execute(
                "UPDATE gun_game SET opponent_id = ? WHERE discord_id = ?",
                (new_opponent_id, ctx.author.id),
            )
            cursor.execute(
                "UPDATE gun_game SET opponent_id = ? WHERE discord_id = ?",
                (ctx.author.id, new_opponent_id),
            )
            new_matchups.append(f"<@{ctx.author.id}> vs <@{new_opponent_id}>")

        if new_matchups:
            embed.add_field(
                name="New Matchups", value="\n".join(new_matchups), inline=False
            )
        else:
            embed.add_field(
                name="No New Opponents",
                value="No new opponents at this time.",
                inline=False,
            )
    ...
    @commands.hybrid_command(
        name="gun_game_signup_other",
        description="Sign up another user for the gun game!",
    )
    ...
    async def gun_game_signup_other(
        self,
        ...
        if await is_game_live(db):
            ...
            if available_player:
                ...
                await ctx.send(
                    f"{user.mention} has been paired with <@{available_player_id}>. Good luck!"
                )
            else:
                await ctx.send(
                    f"{user.mention}, you have no new opponent at this time."
                )
    ...
    @commands.hybrid_command(
        name="gun_game_signup", description="Sign up for the gun game!"
    )
    @app_commands.describe(rsn="Your RuneScape name.")
    @app_commands.describe(proof="Screenshot of your participation proof")
    async def gun_game_signup(
        self, ctx: commands.Context, rsn: str, proof: discord.Attachment
    ):
        # Check if the user already signed up and don't let them sign up twice :)
        db = await get_db(ctx)
        conn = sqlite3.connect(db)
        ...
        if await is_game_live(db):
            ...
            if available_player:
                ...
                await ctx.send(
                    f"<@{ctx.author.id}> has been paired with <@{available_player_id}>. Good luck!"
                )
            else:
                await ctx.send(
                    f"<@{ctx.author.id}>, you have no new opponent at this time."
                )
    ...
    @commands.hybrid_command(
        name="upload_bosses", description="Upload a CSV file with boss information."
    )
    async def upload_bosses(self, ctx: commands.Context, csv_file: discord.Attachment):
        ...
        for row in csv_reader:
            name = str(row[0])
            cursor.execute(
                "INSERT OR REPLACE INTO gun_game_bosses (name) VALUES (?)", (name,)
            )
    ...
    @commands.hybrid_command(
        ...
        description="Get your current task based on your level.",
    )
    async def get_current_task(self, ctx: commands.Context):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT level FROM gun_game WHERE discord_id = ?", (ctx.author.id,)
        )
        result = cursor.fetchone()
        conn.close()
        if not result:
    ...
    @commands.hybrid_command(
        name="knife", description="Knife a user, moving them back one level."
    )
    @app_commands.describe(
        discord_name="The Discord name of the user you want to knife."
    )
    @app_commands.describe(proof="Screenshot of your proof")
    async def knife(
        self, ctx: commands.Context, discord_name: str, proof: discord.Attachment
    ):
        db = await get_db(ctx)
        if not await is_game_live(db):
            await ctx.reply("The game is not currently live.")
        ...
        cursor.execute(
            "SELECT * FROM game_actions WHERE action = 'knife' AND discord_id = ? AND level = ?",
        ...
        if target_level > 0:
            ...
            cursor.execute(
                "UPDATE gun_game SET level = ? WHERE discord_id = ?",
                (new_level, user_id),
            )
            cursor.execute(
                "INSERT INTO game_actions (discord_id, action, level, timestamp) VALUES (?, ?, ?, ?)",
                (user_id, "knife", target_level, datetime.now().isoformat()),
            )

            conn.commit()
            await ctx.send(
                f"<@{ctx.author.id}> has knifed <@{user_id}>, moving them back to level {new_level}."
            )
        else:
            await ctx.reply(
                "The target user is already at the lowest level and cannot be knifed further."
            )
